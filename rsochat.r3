...
grab-icons: func [url
    /local icon-bar name image-url image link
] [
    icon-bar: copy []
    ...
    parse page [thru "update_user"
        some [
            ...
            (
                ; ?? image-url
                either #"!" = first image-url [
                    ; imgur.com .. jpg
                    remove image-url
                    either find image-url ".png" [
                        ;print "png"
                        link: to-url rejoin [image-url "?g&s=32&end=.png"]
                    ] [
                        ;print "jpeg"
                        link: to-url rejoin [image-url "?g&s=32&end=.jpg"]
                    ]
                    image: load link
                    repend icon-bar [image name]
                    repend/only icon-bar ['set-face 'chat-area rejoin ["@" replace/all name " " "" " "] 'focus 'chat-area]
                ] [
                    if parse image-url [some gravatar-rule] [
                        ; gravatar .. png
                        ;print "gravatar"
                        if error? try [
                            image: load rejoin [http://www.gravatar.com/avatar/ image-url "?s=32&d=identicon&r=PG&end=.png"]
                        ] [
                            image: load rejoin [http://www.gravatar.com/avatar/ image-url "?s=32&d=identicon&r=PG&end=.jpg"]
                        ]
                        repend icon-bar [image name]
                        repend/only icon-bar ['set-face 'chat-area rejoin ["@" replace/all name " " "" " "] 'focus 'chat-area]
                    ]
